...
public class JogadorConverter implements TimesConverter<Jogador, JogadorDTO> {
    ...
    public JogadorDTO entidadeParaDto(Jogador entity) {
        return new JogadorDTO(entity.getId(), entity.getNome(), entity.getPosicao(), entity.getIdade(), entity.getClube());
    }

    @Override
    public Jogador dtoParaEntidade(JogadorDTO dto) {
        return new Jogador(dto.getId(), dto.getNome(), dto.getPosicao(), dto.getIdade(), dto.getClube());
    }
